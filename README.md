...
OpenAPI Docs (JSON bruto): http://localhost:8080/v3/api-docs
TIP
...
Crie um Banco de Dados PostgreSQL na plataforma.
Crie um Web Service apontando para o seu repositório GitHub.
Defina o Build Command: mvn clean package -DskipTests (ou utilize o Dockerfile que está na raiz do projeto).
Defina o Start Command: java -jar target/agendamento-0.0.1-SNAPSHOT.jar (se usar build do Maven padrão).
Configure as seguintes Environment Variables nas opções do Web Service:
...
👨‍💻 Autor
Bruno Barretto - LinkedIn | GitHub
